...
public class Streams {
	public static void main(String[] args) {
		...
		System.out.println(
				...
				countDoctors("Andrews", "Fink", "Rothenberg", "Guass", "Alderman")));
		
		Stream<Integer> integers = Stream.concat(Stream.of(1,2), Stream.of(3,4));
	...
	public static void helloDoctors(final String... names) {
		Arrays.stream(names)
	...
	private static double _average (final double... terms) {
		return Arrays.stream(terms)
				.reduce(0, (double total, double current) -> total += current) /terms.length;
	}
}
